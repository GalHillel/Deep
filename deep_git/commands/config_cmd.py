"""
deep_git.commands.config_cmd
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
``deepgit config [--global] <key> [<value>]`` command implementation.
"""
...
def run(args) -> None:  # type: ignore[no-untyped-def]
    ...
    try:
        ...
        if not is_global:
            print("Error: not in a deepgit repository and --global not specified.", file=sys.stderr)
            sys.exit(1)
        config = Config(None)
