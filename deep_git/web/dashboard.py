...
from deep_git.core.objects import read_object, Commit, Tree, Blob, Tag
...
class DashboardHandler(SimpleHTTPRequestHandler):
    ...
    def _handle_get(self):
        if self.path == "/" or self.path == "/index.html":
            ...
            self._json_response(_gather_log(self.dg_dir))
        elif self.path == "/api/refs":
            self._json_response(_gather_refs(self.dg_dir))
        elif self.path.startswith("/api/object/"):
            sha = self.path.split("/")[-1]
            self._json_response(_object_detail(self.dg_dir, sha))
        # ── Multi-Repo API ──────────────────────────────────────────
        ...
        # ── Heatmap API ─────────────────────────────────────────────
            ...
            from deep_git.ai.analyzer import score_complexity
            ...
            from deep_git.core.search import search_history
