...
class SandboxRunner:
    ...
    def _build_env(self) -> Dict[str, str]:
        ...
        for var in ("PATH", "SYSTEMROOT", "TEMP", "TMP", "HOME", "PYTHONPATH"):
            if var in os.environ:
                env[var] = os.environ[var]

        # Set sandbox markers
        env["DEEPGIT_SANDBOX"] = "1"
        env["DEEPGIT_SANDBOX_ALLOWED_PATHS"] = os.pathsep.join(
    ...
    def run(self, script_path: Path, args: Optional[List[str]] = None,
        ...
        if cwd is None:
            sandbox_cwd = Path(tempfile.mkdtemp(prefix="deepgit_sandbox_"))
        else:
            sandbox_cwd = cwd
