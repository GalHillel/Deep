...
def test_ai_review_findings(ai_review_repo):
    ...
    subprocess.run([sys.executable, "-m", "deep_git.main", "add", "main.py"], cwd=repo, env=env, check=True)
    
    result = subprocess.run(
        [sys.executable, "-m", "deep_git.main", "ai", "review"],
...
def test_ai_predict_merge_cli(ai_review_repo):
    repo, env = ai_review_repo
    result = subprocess.run(
        [sys.executable, "-m", "deep_git.main", "ai", "predict-merge"],
        cwd=repo, env=env, capture_output=True, text=True, check=True
    )
