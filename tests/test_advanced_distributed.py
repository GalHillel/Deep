...
def test_divergent_push_detection(tmp_path, env):
    ...
    from deep_git.network.sync import SyncEngine
    repo = tmp_path / "server"
    repo.mkdir()
    subprocess.run([sys.executable, "-m", "deep_git.main", "init"], cwd=repo, env=env, check=True)
    (repo / "a.txt").write_text("init")
    subprocess.run([sys.executable, "-m", "deep_git.main", "add", "a.txt"], cwd=repo, env=env, check=True)
    subprocess.run([sys.executable, "-m", "deep_git.main", "commit", "-m", "init"], cwd=repo, env=env, check=True)
...
def test_incremental_push_sync(tmp_path, env):
    ...
    server.mkdir()
    subprocess.run([sys.executable, "-m", "deep_git.main", "init"], cwd=server, env=env, check=True)
    (server / "a.txt").write_text("v1")
    subprocess.run([sys.executable, "-m", "deep_git.main", "add", "a.txt"], cwd=server, env=env, check=True)
    subprocess.run([sys.executable, "-m", "deep_git.main", "commit", "-m", "v1"], cwd=server, env=env, check=True)
    ...
    subprocess.run([sys.executable, "-m", "deep_git.main", "add", "b.txt"], cwd=server, env=env, check=True)
    subprocess.run([sys.executable, "-m", "deep_git.main", "commit", "-m", "v2"], cwd=server, env=env, check=True)
