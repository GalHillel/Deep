...
def test_distributed_workflow(tmp_path: Path, env: dict[str, str]):
    ...
    server_root.mkdir()
    subprocess.run([sys.executable, "-m", "deep.main", "init"], cwd=server_root, env=env, check=True)
    ...
    try:
        ...
        subprocess.run([sys.executable, "-m", "deep.main", "clone", f"127.0.0.1:{port}", str(client2_root)], env=env, check=True)
        
        # Fetch the new SHA from server
        subprocess.run([sys.executable, "-m", "deep.main", "fetch", f"127.0.0.1:{port}", client1_head], cwd=client2_root, env=env, check=True)
