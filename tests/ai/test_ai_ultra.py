...
def ultra_repo(tmp_path):
    ...
    subprocess.run([sys.executable, "-m", "deep.cli.main", "add", "engine.py", "config.json"],
...
def test_branch_name_from_description():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        env = os.environ.copy()
        env["PYTHONPATH"] = str(Path.cwd() / "src")
        subprocess.run([sys.executable, "-m", "deep.cli.main", "init"], cwd=tmp, env=env, check=True)
        ai = DeepAI(Path(tmp))
