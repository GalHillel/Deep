...
from deep_git.commands.commit_cmd import run as commit_run
from deep_git.core.repository import DEEP_GIT_DIR, init_repo
...
def test_crash_mid_commit_branch_recovery(tmp_path: Path):
    ...
    from deep_git.commands.add_cmd import run as add_run
    
    with mock.patch("deep_git.commands.add_cmd.find_repo", return_value=tmp_path):
        with mock.patch("deep_git.commands.commit_cmd.find_repo", return_value=tmp_path):
            class AddArgs:
                files = [str(file1)]
            add_run(AddArgs())
    ...
    original_update_branch = __import__("deep_git.core.refs", fromlist=["update_branch"]).update_branch
    ...
    with mock.patch("deep_git.commands.commit_cmd.update_branch", side_effect=crashing_update_branch):
        with mock.patch("deep_git.core.refs.update_head", side_effect=crashing_update_branch):
            with mock.patch("deep_git.commands.add_cmd.find_repo", return_value=tmp_path):
                with mock.patch("deep_git.commands.commit_cmd.find_repo", return_value=tmp_path):
