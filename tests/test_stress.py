...
from deep_git.core.refs import resolve_head, list_branches, list_tags
from deep_git.core.txlog import TransactionLog
from deep_git.core.telemetry import TelemetryCollector
...
def test_rapid_commits(stress_repo):
    ...
    for i in range(50):
        ...
        subprocess.run([sys.executable, "-m", "deep_git.main", "add", f"f{i}.txt"],
                       cwd=repo, env=env, check=True)
        subprocess.run([sys.executable, "-m", "deep_git.main", "commit", "-m", f"c{i}"],
                       cwd=repo, env=env, check=True)
...
def test_multi_branch_stress(stress_repo):
    ...
    subprocess.run([sys.executable, "-m", "deep_git.main", "add", "base.txt"], cwd=repo, env=env, check=True)
    subprocess.run([sys.executable, "-m", "deep_git.main", "commit", "-m", "base"], cwd=repo, env=env, check=True)

    for i in range(10):
        subprocess.run([sys.executable, "-m", "deep_git.main", "branch", f"feat-{i}"],
                       cwd=repo, env=env, check=True)
...
def test_doctor_after_stress(stress_repo):
    ...
    subprocess.run([sys.executable, "-m", "deep_git.main", "add", "x.txt"], cwd=repo, env=env, check=True)
    subprocess.run([sys.executable, "-m", "deep_git.main", "commit", "-m", "x"], cwd=repo, env=env, check=True)
...
def test_telemetry_summary_after_stress(stress_repo):
    ...
    for i in range(10):
        ...
        subprocess.run([sys.executable, "-m", "deep_git.main", "add", f"s{i}.txt"],
                       cwd=repo, env=env, check=True)
        subprocess.run([sys.executable, "-m", "deep_git.main", "commit", "-m", f"s{i}"],
                       cwd=repo, env=env, check=True)
...
def test_ai_under_load(stress_repo):
    ...
    for i in range(5):
        ...
        subprocess.run([sys.executable, "-m", "deep_git.main", "add", f"ai{i}.py"],
                       cwd=repo, env=env, check=True)
        subprocess.run([sys.executable, "-m", "deep_git.main", "commit", "-m", f"func {i}"],
                       cwd=repo, env=env, check=True)
